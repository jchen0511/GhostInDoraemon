...
class AutoDrive(object):
    ...
    def on_dashboard(self, src_img, last_steering_angle, speed, throttle, info):
        ...
        #current_angle = ImageProcessor.find_steering_angle_by_line(track_img,
        #last_steering_angle, debug = self.debug)
        ...
        if self.debug:
            ...
            logit("steering PID: %0.2f (%0.2f) => %0.2f (%0.2f)" % (current_angle, ImageProcessor.ImageProcessor.rad2deg(current_angle), steering_angle, ImageProcessor.ImageProcessor.rad2deg(steering_angle)))
